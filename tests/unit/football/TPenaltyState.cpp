...
std::vector<std::string> TPenaltyState::ObtainedResults() const noexcept
{
	...
	{
		result.push_back( std::string{ CPenaltyState::JSON_OUTCOME } + ": " + std::string{ ToString( penaltyState.GetChanceOutcome() ) } );
		result.push_back( std::string{ CPenaltyState::JSON_STOPPER } + ": " + penaltyState.GetGoalkeeper() );
		result.push_back( std::string{ CPenaltyState::JSON_SHOOTER } + ": " + penaltyState.GetShooter() );
