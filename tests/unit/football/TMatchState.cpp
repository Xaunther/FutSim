...
std::vector<std::string> TMatchState::ObtainedResults() const noexcept
{
	...
	{
		result.push_back( std::string{ CMatchState::JSON_MANDATORY_PERIOD_STATES } + ": " +
			std::to_string( matchState.GetMandatoryPlayTimeState().GetStates().size() ) );
		if( matchState.GetExtraTimeState() )
			result.push_back( std::string{ CMatchState::JSON_EXTRA_PERIOD_STATES } + ": " +
				std::to_string( matchState.GetExtraTimeState()->GetStates().size() ) );
		if( matchState.GetPenaltyShootoutState() )
			result.push_back( std::string{ CPenaltyShootoutState::JSON_KEY } + ": " +
